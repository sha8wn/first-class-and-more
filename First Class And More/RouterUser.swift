...
enum RouterUser: URLRequestConvertible {
    ...
    case login(email: String, password: String, devicePushToken: String)
    ...
    var method: HTTPMethod {
        switch self {
            case .getPasswordSalt, .getSettings, .checkUserToken, .checkSubscriber:
                return .get
            case .login, .forgotPassword, .subscribe, .register:
                return .post
        }
    }
    ...
    var params: Parameters {
        switch self {
            ...
            case .login(let email, let password, let devicePushToken):
                return [
                    "login": email,
                    "password": password,
